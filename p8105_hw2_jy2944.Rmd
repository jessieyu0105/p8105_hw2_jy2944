...
* `r transit_data %>% distinct(line, station_name, .keep_all = TRUE) %>% filter(ada == TRUE) %>% nrow()` distinct stations are ADA compliant.
...
* Of the stations that serve the A train, `r transit_data %>% distinct(line, station_name, .keep_all = TRUE) %>% filter(route_name == "A") %>% filter(ada == TRUE) %>% nrow()` stations are ADA compliant.
...
Data frame `precipitation_data` contains information related to precipitation between January 2016 and December 2017. There is `r nrow(precipitation_data)` observations and `r ncol(precipitation_data)` in total. Variables indlude month, year, and precipitaion for each month.
